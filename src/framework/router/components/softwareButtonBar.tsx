...
export function SoftwareKeys({ keymap }: { keymap: KeyBinding[] }) {
  return (
    <div class='software-keys' id='software-keys'>
      <button tabIndex={-1} onMouseDown={emulateKeyPress.bind(null, Key.LSK)}>
        {keymap?.find(({ key }) => key === Key.LSK)?.label}
      </button>
      <button
        tabIndex={-1}
        class='middle'
...
function emulateKeyPress(key: string, ev: MouseEvent) {
  ...
  var keyboardEvent: KeyboardEvent = new KeyboardEvent('keydown', {
    key,
    bubbles: true,
  })
